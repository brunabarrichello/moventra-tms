...
CREATE TABLE audit.audit_events (
    ...
    CONSTRAINT fk_audit_events_actor_user
        ...
        ON UPDATE RESTRICT
        ON DELETE RESTRICT,
    CONSTRAINT fk_audit_events_actor_membership
    ...
    CONSTRAINT fk_audit_events_company
        ...
        ON UPDATE RESTRICT
        ON DELETE RESTRICT,
    CONSTRAINT fk_audit_events_branch
    ...
    CONSTRAINT ck_audit_events_category CHECK (
        category ~ '^[a-z][a-z0-9_.-]{1,63}$'
    ),
    CONSTRAINT ck_audit_events_action CHECK (
        action ~ '^[a-z][a-z0-9_.-]{1,127}$'
    ),
    CONSTRAINT ck_audit_events_entity_type CHECK (
        entity_type ~ '^[a-z][a-z0-9_.-]{1,127}$'
    ),
    CONSTRAINT ck_audit_events_entity_id CHECK (
    ...
    CONSTRAINT ck_audit_events_before_object CHECK (
        jsonb_typeof(before_data) = 'object'
        AND octet_length(before_data::text) <= 65536
    ),
    CONSTRAINT ck_audit_events_after_object CHECK (
        jsonb_typeof(after_data) = 'object'
        AND octet_length(after_data::text) <= 65536
    ),
    CONSTRAINT ck_audit_events_metadata_object CHECK (
...
COMMENT ON TABLE audit.audit_events IS
    'Tenant-scoped append-only central audit trail. Payloads must be minimized and redacted before insertion; secrets and authentication tokens are forbidden.';
COMMENT ON COLUMN audit.audit_events.metadata IS
    'Redacted structured context only. Never store passwords, credentials, access/refresh tokens, authorization headers or raw session secrets.';
